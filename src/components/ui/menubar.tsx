...
const SetPlacementContext = React.createContext<Dispatch<SetStateAction<Ariakit.MenuProviderProps['placement']>>>(
    () => {}
);
...
export const Menubar = React.forwardRef<HTMLDivElement, MenubarProps>(function Menubar(props, ref) {
    const [shift, setShift] = React.useState(0);
    const [placement, setPlacement] = React.useState<Ariakit.MenuProviderProps['placement']>('bottom');
    return (
        <Ariakit.Menubar ref={ref} {...props} className={cn('flex items-center gap-1', props.className)}>
            <SetShiftContext.Provider value={setShift}>
                <SetPlacementContext.Provider value={setPlacement}>
                    <Ariakit.MenuProvider animated placement={placement} showTimeout={100} hideTimeout={250}>
                        {props.children}
                        <Ariakit.Menu
                            ...
                            unmountOnHide
                            className={cn(
                                'relative z-50 max-h-[var(--popover-available-height)] max-w-[var(--popover-available-width)] rounded-md border border-solid p-4 bg-popover transition-opacity opacity-0 data-enter:opacity-100 focus-visible:text-primary data-enter:animate-in data-enter:zoom-in-90',
                                props.className
                            )}
                        >
...
export const Menu = React.forwardRef<HTMLDivElement, MenuProps>(function Menu(
    ...
    const item = (
        ...
        >
            {label}
            {!!children && <Ariakit.MenuButtonArrow className="transition-transform group-aria-expanded:rotate-180" />}
        </Ariakit.MenuItem>
    );
...
export const MenuItem = React.forwardRef<HTMLDivElement, MenuItemProps>(function MenuItem({ ...props }, ref) {
    ...
    return (
        <Ariakit.MenuItem
            ...
            {...props}
            className={cn(
                'flex flex-col items-start gap-1 rounded-sm p-4 hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground focus:outline-none cursor-pointer',
                props.className
            )}
...
export const MenuGroup = React.forwardRef<HTMLDivElement, MenuGroupProps>(function MenuGroup({ label, ...props }, ref) {
    return (
        <Ariakit.MenuGroup ref={ref} {...props} className={cn('flex flex-col', props.className)}>
